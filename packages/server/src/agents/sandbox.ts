...
export function buildNsjailArgs(config: SandboxConfig): string[] {
  ...
  // Companion tools can live next to a CLI's real entrypoint rather than in
  // the usual global bin dir. Codex ships an `rg` wrapper there.
  ...
  // Home directory for sandbox user
  ...
  // PATH must include companion dirs exposed by global CLIs, such as Codex's
  // bundled ripgrep wrapper.
